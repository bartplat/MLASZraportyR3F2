...
1. Informacje o badaniu
    ...
    b) Raport opiera się na wynikach badania internetowego przeprowadzonego w okresie od listopada 2021 roku do lutego 2022 roku. Badanie wśród absolwentów Państwa `r case_when(szk$TYP_SZK[[1]] %in% 1 ~ "branżowej szkoły pierwszego stopnia", szk$TYP_SZK[[1]] %in% 2 ~ "technikum", szk$TYP_SZK[[1]] %in% 3 ~ "szkoły policealnej")` realizowała firma `r szk$firma[[1]]`.
    c) Zaproszenie do wypełnienia ankiety on-line skierowano do `r szk$l_ucz_f1[[1]]` absolwentów, którzy w roku szkolnym 2020/2021 wzięli udział w I fali badania jako uczniowie ostatniej klasy Państwa szkoły. To samo badanie zrealizowano wówczas we wszystkich szkołach kształcących w zawodach szkolnictwa branżowego, o ile ich dyrektorzy wyrazili chęć udziału w badaniu.
        
        Raporty z tego badania udostępniliśmy poszczególnym szkołom w styczniu i lutym 2022 roku, wysyłając link z adresu raporty@losyabsolwentow.pl.
    d) Ankietę, której wyniki przedstawiamy w niniejszym raporcie, wypełniło `r szk$l_abs[[1]]` absolwentów Państwa szkoły, tj. `r round(szk$l_abs[[1]] / szk$l_ucz_f1[[1]] * 100)`% uczniów, którzy wzięli udział w I fali badania. **Wyników nie należy uogólniać na pozostałych absolwentów Państwa szkoły.**
...
if (szk$l_abs >= progLiczebnosci) {
  cat(
    "    a) Wyniki dotyczące absolwentów Państwa szkoły zestawiono z odpowiedziami absolwentów innych ",
    ...
    "    b) Minimalna liczebność grupy absolwentów Państwa szkoły, dla której przedstawiane są poszczególne wyniki w raporcie, wynosi 10 osób. Jeżeli wśród absolwentów szkoły nie udało się uzyskać odpowiedzi na pytanie od minimum 10 respondentów, wynik nie jest prezentowany.\n",
...
3) Absolwenci, których dotyczy raport
    ...
    a) Osoby, które w roku 2020/2021 wzięły udział w badaniu jako uczniowie ostatnich klas `r szk$formy[[1]]$typ_szk_dop_mn`, a następnie jako absolwenci w badaniu realizowanym od listopada 2021 roku do lutego 2022 roku.
    b) Liczba absolwentów Państwa szkoły, których dotyczy raport: `r szk$l_abs[[1]]`, w tym kobiet: `r szk$l_kobiet[[1]]`.

```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis', fig.align=wyrownanieTabWykr}
if (szk$zaw_licz[[1]]$n > 0) {
...
if (szk$praca_zarobkowa[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "Na poszczególnych wykresach dane dotyczące absolwentów Państwa szkoły są zestawiane z danymi dotyczącymi absolwentów innych ",
    szk$formy[[1]]$typ_szk_dop_mn,
    " z województwa ",
    szk$formy[[1]]$woj_nazwa_dop,
...
if (szk$praca_zarobkowa[[1]]$n >= progLiczebnosci) {
  ...
  ggplot(tab, aes(x = reorder(typ, desc(typ)), y = rozklad)) +
    geom_bar(width = 0.75, stat = "identity", colour = "white", fill = "#4daf49") +
...
if (szk$praca_zarobkowa[[1]]$n >= progLiczebnosci) {
  ...
  cat(
    "*\\textcolor{gray}{Wykres przedstawia odpowiedzi ",
...
## 2.3. Bierność edukacyjna i zawodowa
...
if (szk$neet[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "\n\n\\textcolor{gray}{Bierność edukacyjna i zawodowa jest tutaj rozumiana jako brak w danym miesiącu co najmniej dwutygodniowego epizodu nauki, wykonywania pracy zarobkowej oraz zarejestrowanego bezrobocia.}",
    sep = ""
  )
} else {
  cat("Odpowiedzi uzyskano od mniej niż 10 absolwentów Państwa szkoły - grupa ta jest zbyt mało liczna, żeby przedstawić wyniki.")
...
if (szk$neet[[1]]$n >= progLiczebnosci) {
  ...
  cat(
    ...
    if (nrow(filter(miesiace["value"], value %in% min(value))) %in% 1) {
      cat(
        "Odsetek biernych edukacyjnie i zawodowo absolwentów Państwa szkoły w ciągu rozpatrywanego okresu był najniższy ",
    ...
    if (nrow(filter(miesiace["value"], value %in% min(value))) %in% 4) {
      cat(
        "Odsetek biernych edukacyjnie i zawodowo absolwentów Państwa szkoły nie różnił się w zależności od miesiąca.\n\n",
        sep = ""
      )
    },
    sep = ""
...
if (szk$status_2021_10[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "% objętych badaniem absolwentów Państwa szkoły, poświęciło po co najmniej dwa tygodnie zarówno na pracę zarobkową, jak i na naukę ",
    ifelse(szk$TYP_SZK[[1]] %in% 1,
           paste0("w kolejnej szkole"),
           paste0("w szkole lub na studiach")),
    ". Z kolei ",
...
if (szk$status_2021_10[[1]]$n >= progLiczebnosci) {
  ...
  cat(
    "*\\textcolor{gray}{Wykres przedstawia rozkład zmiennej utworzonej na podstawie pytań: “Proszę spojrzeć na poniższy kalendarz i zaznaczyć miesiące, w których w sumie przez co najmniej dwa tygodnie (bez względu na liczbę godzin w tygodniu): studiował(a) P. lub uczył(a) się P. w szkole / pracował(a) P. we własnym gospodarstwie rolnym lub w gospodarstwie P. rodziny / pracował(a) P. zarobkowo u pracodawcy (na dowolnych zasadach) / pracował(a) P. na własny rachunek, czyli nie u pracodawcy?”. Respondenci, w odniesieniu do każdego miesiąca, wskazywali odpowiedź “Tak” lub “Nie”. Wykres uwzględnia odpowiedzi dotyczące października, udzielone przez ",
...
if (szk$TYP_SZK[[1]] %in% 1) {
  if (szk$eduk_kontyn[[1]]$n >= progLiczebnosci) {
    if (!(round(szk$eduk_kontyn[[1]]$any_bs1 * 100) %in% 0)) {
      cat(
        "Pomiędzy lipcem 2021 roku a momentem wypełnienia ankiety (badanie było realizowane w okresie od listopada 2021 do lutego 2022 roku), spośród absolwentów Państwa szkoły naukę w branżowej szkole drugiego stopnia, w liceum dla dorosłych lub na kwalifikacyjnym kursie zawodowym rozpoczęło ",
        round(szk$eduk_kontyn[[1]]$any_bs1 * 100),
        "%.",
        sep = ""
      )
    } else {
      cat("Żaden z absolwentów Państwa szkoły nie kontynuował edukacji w branżowej szkole drugiego stopnia, w liceum dla dorosłych lub na kwalifikacyjnym kursie zawodowym.")
    }
  } else {
    cat("Odpowiedzi uzyskano od mniej niż 10 absolwentów Państwa szkoły - grupa ta jest zbyt mało liczna, żeby przedstawić wyniki.")
  }
...
if (szk$TYP_SZK[[1]] %in% 1) {
  if (szk$eduk_kontyn[[1]]$n >= progLiczebnosci) {
    # if (!(round(szk$eduk_kontyn[[1]]$any_bs1 * 100) %in% 0)) {
      ...
      tytul = wrapper(paste0(
        "Wykres ", wyk_num(),
        ": Odsetek absolwentów, którzy po ukończeniu branżowej szkoły pierwszego stopnia kontynuowali naukę w dowolnej formie"),
        width = 80)
  ...
  if (szk$eduk_kontyn[[1]]$n >= progLiczebnosci) {
    # if (!(round(szk$eduk_kontyn[[1]]$any_bs1 * 100) %in% 0)) {
      cat(
        ...
        " z województwa ",
        szk$formy[[1]]$woj_nazwa_dop,
        ". Respondentom zadano pytanie “Czy po zakończeniu nauki w (...) branżowej szkole I stopnia uczestniczył(a) Pan(i) w rekrutacji: (...) do branżowej szkoły II stopnia, (...) liceum dla dorosłych, (...) na kwalifikacyjny kurs zawodowy?”. Respondenci wskazywali odpowiedź “Tak i rozpocząłem(am) tam naukę”, “Tak, ale nie rozpocząłem(am) tam nauki” lub “Nie”. Wykres uwzględnia pierwszą z tych odpowiedzi.}*\n\n",
        sep = ""
      )
      
      cat(
        "W branżowej szkole drugiego stopnia podjęło dalszą naukę ",
...
if (szk$TYP_SZK[[1]] %in% 1) {
  if (szk$eduk_kontyn[[1]]$n >= progLiczebnosci) {
    ...
    #   if (szk$eduk_kontyn[[1]]$bs2 > 0) {
        ...
        tytul = wrapper(paste0(
          "Wykres ", wyk_num(),
          ": Odsetek absolwentów, którzy po ukończeniu branżowej szkoły pierwszego stopnia kontynuowali naukę w branżowej szkole drugiego stopnia"),
          width = 80)
...
if (szk$TYP_SZK[[1]] %in% 1) {
  if (szk$eduk_kontyn[[1]]$n >= progLiczebnosci) {
    ...
    #   if (szk$eduk_kontyn[[1]]$lodd > 0) {
        ...
        tytul = wrapper(paste0(
          "Wykres ", wyk_num(),
          ": Odsetek absolwentów, którzy po ukończeniu branżowej szkoły pierwszego stopnia kontynuowali naukę w liceum dla dorosłych"),
          width = 80)
...
if (szk$TYP_SZK[[1]] %in% 1) {
  if (szk$eduk_kontyn[[1]]$n >= progLiczebnosci) {
    ...
    #   if (szk$eduk_kontyn[[1]]$kkz > 0) {
        ...
        tytul = wrapper(paste0(
          "Wykres ", wyk_num(),
          ": Odsetek absolwentów, którzy po ukończeniu branżowej szkoły pierwszego stopnia kontynuowali naukę na kwalifikacyjnym kursie zawodowym"),
          width = 80)
...
if (szk$TYP_SZK[[1]] %in% 3) {
  if (szk$eduk_kontyn[[1]]$n >= progLiczebnosci) {
    if (!(round(szk$eduk_kontyn[[1]]$any_tech * 100) %in% 0)) { # dla spolic i tech to jest ten sam zestaw możliwości kontynuowania edukacji, więc to "any_tech" jest ok
      cat(
        "Pomiędzy lipcem 2021 roku a momentem wypełnienia ankiety (badanie było realizowane w okresie od listopada 2021 do lutego 2022 roku), spośród absolwentów Państwa szkoły naukę na studiach lub na kwalifikacyjnym kursie zawodowym rozpoczęło ",
        round(szk$eduk_kontyn[[1]]$any_tech * 100),
        "%.",
        sep = ""
      )
    } else {
      cat("Żaden z absolwentów Państwa szkoły nie kontynuował edukacji na studiach lub na kwalifikacyjnym kursie zawodowym.")
    }
  } else {
    cat("Odpowiedzi uzyskano od mniej niż 10 absolwentów Państwa szkoły - grupa ta jest zbyt mało liczna, żeby przedstawić wyniki.")
  }
...
if (szk$TYP_SZK[[1]] %in% 3) {
  if (szk$eduk_kontyn[[1]]$n >= progLiczebnosci) {
    # if (!(round(szk$eduk_kontyn[[1]]$any_tech * 100) %in% 0)) {
      ...
      tytul = wrapper(paste0(
        "Wykres ", wyk_num(),
        ": Odsetek absolwentów, którzy po ukończeniu szkoły policealnej kontynuowali naukę w dowolnej formie"),
        width = 80)
  ...
  if (szk$eduk_kontyn[[1]]$n >= progLiczebnosci) {
    # if (!(round(szk$eduk_kontyn[[1]]$kkz * 100) %in% 0)) {
      cat(
        ...
        " z województwa ",
        szk$formy[[1]]$woj_nazwa_dop,
        ". Respondentom zadano pytanie “Czy po zakończeniu nauki w (...) szkole policealnej uczestniczył(a) Pan(i) w rekrutacji: (...) do szkoły policealnej, (...) na studia, (...) na kwalifikacyjny kurs zawodowy?”. Respondenci wskazywali odpowiedź “Tak i rozpocząłem(a) tam naukę”, “Tak, ale nie rozpocząłem(am) tam nauki” lub “Nie”. Wykres uwzględnia pierwszą z tych odpowiedzi.}*",
        sep = ""
      )
    # }
  }
...
if (szk$TYP_SZK[[1]] %in% 3) {
  if (szk$eduk_kontyn[[1]]$n >= progLiczebnosci) {
    if (!(round(szk$eduk_kontyn[[1]]$kkz * 100) %in% 0)) {
      cat(
        "Spośród absolwentów Państwa szkoły naukę na kwalifikacyjnym kursie zawodowym rozpoczęło ",
        round(szk$eduk_kontyn[[1]]$kkz * 100),
        "%.",
        sep = ""
      )
...
if (szk$TYP_SZK[[1]] %in% 3) {
  if (szk$eduk_kontyn[[1]]$n >= progLiczebnosci) {
    if (!(round(szk$eduk_kontyn[[1]]$stud_ogolem * 100) %in% 0)) {
      cat(
        "Studentami zostało ",
        round(szk$eduk_kontyn[[1]]$stud_ogolem * 100),
        "% absolwentów Państwa szkoły, z którymi przeprowadzono badanie.\r\n\r\n",
      ...
      cat("Żaden z absolwentów Państwa szkoły nie kontynuował edukacji na studiach wyższych.")
    }
  }
}
```
...
if (szk$TYP_SZK[[1]] %in% 3) {
  if (szk$eduk_kontyn[[1]]$n >= progLiczebnosci) {
    # if (!(round(szk$eduk_kontyn[[1]]$stud_ogolem * 100) %in% 0)) {
      ...
      tytul = wrapper(paste0(
        "Wykres ", wyk_num(),
        ": Odsetek absolwentów, którzy po ukończeniu szkoły policealnej kontynuowali naukę na studiach wyższych"),
        width = 80)
...
if (szk$TYP_SZK[[1]] %in% 3) {
  if (szk$eduk_kontyn[[1]]$n >= progLiczebnosci) {
    ...
    #   if (szk$eduk_kontyn[[1]]$spolic > 0) {
        ...
        cat(
          "W kolejnej szkole policealnej podjęło dalszą naukę ",
          round(szk$eduk_kontyn[[1]]$spolic * 100),
          "% absolwentów Państwa szkoły objętych badaniem.\r\n\r\n",
          sep = ""
        )
        ...
        tytul = wrapper(paste0(
          "Wykres ", wyk_num(),
          ": Odsetek absolwentów, którzy po ukończeniu szkoły policealnej kontynuowali naukę w kolejnej szkole policealnej"),
          width = 80)
...
if (szk$zgodnosc_pracy[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    " z województwa ",
    szk$formy[[1]]$woj_nazwa_dop,
    ". Respondentom zadano pytanie “Czy od czasu ukończenia [typ szkoły, w której uczył się respondent], czyli od lipca 2021 roku do [miesiąc poprzedzający miesiąc, w którym respondent wypełnia ankietę] 2021 roku w sumie przez co najmniej dwa tygodnie wykonywał(a) Pan(i) pracę zarobkową w: zawodzie [zawód, w którym kształcił się respondent]? (...)”. Respondenci wskazywali odpowiedź “Tak” lub “Nie”.}*\n\n",
...
if (szk$zgodnosc_pracy[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    " z województwa ",
    szk$formy[[1]]$woj_nazwa_dop,
    ". Respondentom zadano pytanie “Czy od czasu ukończenia [typ szkoły, w której uczył się respondent] w okresie, czyli od lipca 2021 roku do [miesiąc poprzedzający miesiąc, w którym respondent wypełnia ankietę] 2021 roku w sumie przez co najmniej dwa tygodnie wykonywał(a) Pan(i) pracę zarobkową w: zawodzie [zawód, w którym kształcił się respondent]?; innym zawodzie, ale w branży [branża, w której kształcił się respondent]. Respondenci wskazywali odpowiedź “Tak” lub “Nie”. Na wykresie uwzględniono respondentów, którzy odpowiedzieli przecząco na pytanie dotyczące zawodu ale twierdząco na pytanie dotyczące branży.}*",
    sep = ""
  )
}
```
...
if (szk$zad_ogolne[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    if (round(szk$zad_ogolne[[1]]$t2b * 100) == round(szk$zad_ogolne[[1]]$b2b * 100)) {
      ...
      paste0("mniej: ", round(szk$zad_ogolne[[1]]$b2b * 100), "% absolwentów.")
    } else if (round(szk$zad_ogolne[[1]]$t2b * 100) < round(szk$zad_ogolne[[1]]$b2b * 100)) {
      paste0("więcej: ", round(szk$zad_ogolne[[1]]$b2b * 100), "% absolwentów.")
    },
...
if (szk$zad_ogolne[[1]]$n >= progLiczebnosci) {
  ...
  tytul = wrapper(paste0(
    "Wykres ", wyk_num(),
    ": Ogólny poziom zadowolenia absolwentów z podjętej pracy zarobkowej"),
    width = 80)
  ...
  ggplot(tab, aes(x = reorder(typ, desc(typ)), y = value, fill = name)) +
    ...
    scale_fill_manual(values = c("#797979", "#b31cb5", "#b976b9", "#fffdb2", "#a1d669", "#4d9121"))
}
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis', fig.align=wyrownanieTabWykr}
if (szk$zad_ogolne[[1]]$n >= progLiczebnosci) {
  cat("Dane z wykresu przedstawiono w poniższej tabeli.")
  
  tab1 %>% 
    left_join(tab2, by = "name") %>% 
    select(-starts_with(c("lab", "typ"))) %>% 
    mutate(across(starts_with("value"), ~paste0(round(. * 100), "\\%"))) %>% 
    kable("latex",
          booktabs = TRUE,
          escape = FALSE,
          longtable = TRUE,
...
if (szk$zad_rozwoj[[1]]$n >= progLiczebnosci) {
  ...
  tytul = wrapper(paste0(
    "Wykres ", wyk_num(),
    ": Poziom zadowolenia absolwentów z możliwości rozwoju zawodowego w podjętej pracy zarobkowej"),
    width = 80)
...
if (szk$zad_zycie[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    if (round(szk$zad_zycie[[1]]$t2b * 100) == round(szk$zad_zycie[[1]]$b2b * 100)) {
      ...
      paste0("więcej absolwentów: ", round(szk$zad_zycie[[1]]$b2b * 100), "%.")
    },
    " Pozostali wybrali odpowiedź pośrednią (",
...
if (szk$zad_zycie[[1]]$n >= progLiczebnosci) {
  ...
  tytul = wrapper(paste0(
    "Wykres ", wyk_num(),
    ": Poziom zadowolenia absolwentów z możliwości godzenia pracy zarobkowej z życiem poza pracą"),
    width = 80)
...
if (szk$zad_zarobki[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    if (round(szk$zad_zarobki[[1]]$t2b * 100) == round(szk$zad_zarobki[[1]]$b2b * 100)) {
      ...
      paste0("mniej absolwentów: ", round(szk$zad_zarobki[[1]]$b2b * 100), "%.")
    } else if (round(szk$zad_zarobki[[1]]$t2b * 100) < round(szk$zad_zarobki[[1]]$b2b * 100)) {
      paste0("więcej absolwentów: ", round(szk$zad_zarobki[[1]]$b2b * 100), "%.")
    },
    " Pozostali wybrali odpowiedź pośrednią (",
...
if (szk$zad_zarobki[[1]]$n >= progLiczebnosci) {
  ...
  tytul = wrapper(paste0(
    "Wykres ", wyk_num(),
    ": Poziom zadowolenia absolwentów z uzyskiwanych zarobków"),
    width = 80)
...
if (szk$zad_utrzym[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    if (round(szk$zad_utrzym[[1]]$t2b * 100) == round(szk$zad_utrzym[[1]]$b2b * 100)) {
      ...
      paste0("mniej absolwentów: ", round(szk$zad_utrzym[[1]]$b2b * 100), "%.")
    } else if (round(szk$zad_utrzym[[1]]$t2b * 100) < round(szk$zad_utrzym[[1]]$b2b * 100)) {
      paste0("więcej absolwentów: ", round(szk$zad_utrzym[[1]]$b2b * 100), "%.")
    },
    " Pozostali wybrali odpowiedź pośrednią (",
...
if (szk$zad_utrzym[[1]]$n >= progLiczebnosci) {
  ...
  tytul = wrapper(paste0(
    "Wykres ", wyk_num(),
    ": Poziom zadowolenia absolwentów ze stabilności pracy zarobkowej"),
    width = 80)
...
Więcej informacji o monitoringu karier absolwentów znajdą Państwo na stronie:
