...
if (szk$status_2021_10[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "%.\n\n\textcolor{gray}{Dla absolwentów wszystkich szkół rozpatrywano sytuację w październiku 2021 roku, żeby uwzględnić ewentualne rozpoczynanie nauki na studiach po ukończeniu szkół techników i szkół policealnych.}",
    sep = ""
  )
} else {
  cat("Odpowiedzi uzyskano od mniej niż 10 absolwentów Państwa szkoły - grupa ta jest zbyt mało liczna, żeby przedstawić wyniki.")
...
if (szk$zad_ogolne[[1]]$n >= progLiczebnosci) {
  ...
  ggplot(tab, aes(x = typ, y = value, fill = name)) +
    geom_col(width = 0.75, stat = "identity", colour = "white", position = "stack") +
    ...
    theme(legend.position = "bottom",
          ...
          plot.title = element_text(size = 12)) +
    geom_rect(xmin = 0.5, xmax = 2.5, ymin = 0, ymax = 1, colour = "black", alpha = 0, size = 0.25) +
    guides(fill = guide_legend(reverse = FALSE, nrow = 6, byrow = TRUE)) +
    scale_fill_brewer(palette = "RdPu")
}
```
